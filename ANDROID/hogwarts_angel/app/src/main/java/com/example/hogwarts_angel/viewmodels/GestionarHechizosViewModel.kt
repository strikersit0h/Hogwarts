...
class GestionarHechizosViewModel : ViewModel() {
    ...
    fun crearHechizo(hechizo: Hechizo) {
        viewModelScope.launch {
            try {
                val response = api.crearHechizo(hechizo)
    ...
    fun actualizarHechizo(id: Int?, hechizo: Hechizo) {
        viewModelScope.launch {
            try {
                val response = api.actualizarHechizo(id, hechizo)
                _operacionExitosa.postValue(response.isSuccessful)
            } catch (e: Exception) {
                _operacionExitosa.postValue(false)
            }
